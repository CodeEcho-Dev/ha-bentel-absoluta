...
One method per endpoint documented in docs/protocol.md, plus the
long-poll chunk parser and the exception hierarchy. Deliberately has no
knowledge of Tiers/locking/reauth - that orchestration lives in
coordinator.py (see docs/phase5-implementation-guide.md sub-step 4 for
...
class BentelAbsolutaAuthError(BentelAbsolutaError):
    ...
    errorType "wrongPin" - confirmed 2026-09-05 that the credentials PUT
    itself never fails for a bad PIN (always 200, echoing back whatever
    was submitted). This is the *only* signal that means "reauth", and it
    must not be confused with BentelAbsolutaConnectionError even though
    both arrive as the same {"eventType": "deviceError", ...} envelope -
    the discriminator is the errorType field.
    """
...
class BentelAbsolutaBusyError(BentelAbsolutaError):
    ...
    UNREACHABLE is ambiguous - confirmed 2026-09-05 it can also mean a
    simply wrong/nonexistent serial, not just "right after an eviction".
    Both are treated as transient/retry-later by the coordinator; Config
    Flow (which validates a serial for the first time) additionally
    retries once before concluding a serial is actually wrong - see
    docs/phase5-implementation-guide.md sub-step 3.
    """
...
def parse_longpoll_chunk(text: str) -> list[dict[str, Any]]:
    ...
    at once (confirmed in docs/protocol.md, observed up to 3 in one
    chunk) - this is pure, dependency-free parsing so it can be unit
    tested against fixed sample payloads with no live panel needed (see
    docs/phase5-implementation-guide.md sub-step 4's "Testing" section).
    """
    import json
...
class BentelAbsolutaApiClient:
    ...
    async def create_session(self, serial: str) -> str:
        ...
        409 BUSY/UNREACHABLE (see docs/protocol.md's "Session
        exclusivity")."""
    ...
    async def submit_credentials(self, session_id: str, pin: str) -> None:
        ...
        Confirmed 2026-09-05: this always returns 200 regardless of
        whether the PIN is correct - it does not validate anything, it
        just echoes back whatever was submitted. The real PIN check
        happens on the first long-poll event (see poll() below) - do not
        add a status-code check here expecting it to ever fail for a bad
        PIN, it won't.
        """
        resp = await self._request(
            "PUT",
    ...
    async def poll(
        ...
        being branched on, confirmed 2026-09-05 (see docs/protocol.md's
        "Submit panel PIN" and "Connection loss & recovery").
        """
        params = {
            "Content-type": "application/json",
    ...
    # arrives asynchronously over the long-poll - see docs/protocol.md's
    ...
    async def put_group_arming(self, session_id: str, group: str) -> None:
        """Stateless toggle - see docs/protocol.md, there's no explicit
        end-state form for this endpoint."""
    ...
    async def put_settings_notifications(self, session_id: str) -> None:
        ...
        arrived - calling it earlier reliably 404s (docs/protocol.md)."""
        await self._request(
            "PUT",
    ...
    # ---- session-independent (docs/protocol.md's "session exclusivity" -
    # these never compete for the exclusive session lock) ----
...
def generate_client_id() -> str:
    ...
    time, never regenerated afterward (see sub-step 3)."""
